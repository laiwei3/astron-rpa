...
public interface AppMarketVersionDao extends BaseMapper<AppMarketVersion> {
    ...
    Integer insertAppVersionBatch(@Param("latestAppVersionInfo") AppMarketVersion latestAppVersion, @Param("appInfoList")List<AppMarketResource> appInfoList);
    ...
    @Select("select * " +
            ...
            "order by app_version desc")
    List<AppMarketVersion> getAllAppVersionRegardlessDel(@Param("appId") String appId, @Param("marketId") String marketId);
    ...
    List<AppMarketVersion> getAppVersionList(@Param("marketId") String marketId, @Param("appIdList") List<String> appIdList);
